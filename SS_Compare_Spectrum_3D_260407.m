...
%        - 중복되는 3D magnitude figure 제거
%        - Formula 시간영역 재구성 파형을 sin(m*theta + angle) 기준으로 수정
%        - 버전 기록 형식 추가 및 누적 관리 시작
% v1.2 : (2026-04-08)
...
% compare가 figure를 총괄한다. sweep 스크립트는 데이터만 생성한다.
enable_sweep_plots = false;
...
phi_formula_deg = phi_best_list;
Ism_ang_formula = Ism_ang_all;
...
phi_sim_deg_compare = phi_sim_deg;
Ism_ang_simulink = Ism_sim_ang_all;
clear enable_sweep_plots;
...
if exist('Ism_ang_formula', 'var') && exist('Ism_ang_simulink', 'var')
    Ism_ang_formula_plot = rad2deg(Ism_ang_formula(plot_idx, 1:length(delta_list)));
    Ism_ang_simulink_plot = rad2deg(Ism_ang_simulink(plot_idx, 1:length(delta_list)));
    ...
    disp('위상 데이터(Ism_ang_formula, Ism_ang_simulink)를 찾을 수 없습니다.');
end
...
ang_f = Ism_ang_formula(:, idx_target);
...
for m_i = 1:length(m_list)
    ...
    i_m = sqrt(2) * mag_f(m_i) .* sin(m * theta_vec + ang_f(m_i));
...
ang_s = Ism_ang_simulink(:, idx_target);
