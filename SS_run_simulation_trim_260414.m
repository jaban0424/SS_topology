%% SS_run_simulation_trim_260414.m
% =========================================================================
% [버전 기록]
% v1.1 : (2026-04-14 20:40) 비측정 시간 구간을 trim 하도록 수정
% v1.0 : (2026-04-07) SS_topology_design.m 에 통합되어 있던 시뮬레이션 실행 및
%        후처리 분석(Postprocess) 기능을 독립된 스크립트로 분리함.
% =========================================================================
% 설명: SS_topology_design.m의 파라미터를 기반으로 시뮬레이션을 수행하고
%       AC/DC 분석을 진행합니다.
...
%% 4. logsout 추출 및 데이터 구조 최적화 (Trimming)
logs = [];
...
% tStart와 tEnd의 무결성 검사
if tStart > tEnd
    warning(['tStart(%.4f)가 tEnd(%.4f)보다 큽니다. ', ...
             'SS_topology_design.m의 파라미터를 확인하세요.'], tStart, tEnd);
end
...
% 축소된 Dataset만 분석에 사용하고, 원본 out 객체는 즉시 메모리에서 제거
clear out;
evalin('base', 'if exist("out","var"), clear out; end');

% 후속 스크립트가 인식할 수 있도록 변수명을 logsout으로 되돌려 저장
logsout = logs;
...
for k = 1:numel(sigNames)
    ...
    if isDC
        % ---- DC signal handling ----
        [rms_dc, ~] = local_calc_dc_stats(ts, tStart, tEnd);
...
function X = local_calc_f0_phasor_only(ts, f0, tStart, tEnd)
    ...
    x = ts.Data(:);
    if ~isvector(x)
        error('Reference Data가 벡터/행렬 신호임.');
    end
    ...
    x = x(idx);
    if numel(t) < 10
        error('Reference 윈도우 데이터가 너무 적음.');
    end
...
function [rms_dc, mean_dc] = local_calc_dc_stats(ts, tStart, tEnd)
    ...
    x = ts.Data(:);
    if ~isvector(x)
        error('DC Data가 벡터/행렬 신호임.');
    end

    idx = (t >= tStart) & (t <= tEnd);
    x = x(idx);
    if numel(x) < 10
        error('DC 윈도우 데이터가 너무 적음.');
    end
    ...
    for i = 1:logs.numElements
        ...
        if isa(el, 'Simulink.SimulationData.Signal')
            if isa(el.Values, 'timeseries')
                ...
                if isprop(el, 'PortType')
                    newEl.PortType = el.PortType;
                end
                if isprop(el, 'PortIndex')
                    newEl.PortIndex = el.PortIndex;
                end
        ...
        elseif isa(el, 'timeseries')
            ...
            % timeseries가 아닌 element는 (struct, vector 등) 원본 유지
            newLogs = newLogs.addElement(el, elName);
        end
    end
...
% timeseries 내부의 Time/Data를 tStart~tEnd 범위로 잘라서 반환
    ...
    if sum(idx) == 0
        ...
        error(['Trimming 결과 데이터가 0건입니다 (Name: %s).\n', ...
